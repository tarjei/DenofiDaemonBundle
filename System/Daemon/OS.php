...
class OS
{
    ...
    public static function factory($force_os = false, $retried = false)
    {
        $drivers      = array();
        $driversValid = array();
        $class_prefix =  'Denofi\DaemonBundle\System\Daemon\OS\\';;
        ...
        foreach (glob($driver_dir."/*.php") as $driver_path) {
            ...
            if (!class_exists($class, true)) {
                self::$error = "Class ".$class." does not exist";
                return null;
            }
        ...
        if ($force_os !== false) {
            ...
            $use_name = OS::_mostSpecific($driversValid);
        }
        ...
        if (!isset($drivers[$use_name])) {
            ...
            if (!$retried) {
                $obj           = OS::factory(false, true);
                self::$error = "Unable to use driver: ".$use_name." falling ".
                    "back to autodetection.";
            } else {
                $obj = false;
                self::$error = "Unable to use driver even after falling ".
                    "back to autodetection.";
            }
        } else {
    ...
    public function getDataDir()
    {
        ...
        if (!$dir) {
            $try_dir = realpath(dirname(__FILE__)."/OS/Data");
            if (!is_dir($try_dir)) {
                $tried_dirs[] = $try_dir;
            } else {
                $dir = $try_dir;
    ...
    /**
     * Deletes an: 'init.d' script on the filesystem
     *
     * @param array   $properties Contains the daemon properties
     * @param boolean $overwrite  Wether to overwrite when the file exists
     *
     ...
     * @see getAutoRunPath()
     */
    public function deleteAutoRun($properties)
    {
        // Get script path
        if (($path = $this->getAutoRunPath($properties["appName"])) === false) {
            // Explaining errors should have been generated by
        ...
        // Overwrite?
        if (!file_exists($path)) {
            $this->errors[] =  "startup file: '".$path."' does not exist.";
            return false;
        }

        if (!unlink($path)) {
            $this->errors[] =  "startup file: '".$path."' cannot be deleted. Check the permissions";
            return false;
        }
     ...
     * @return string
     */
    static protected function _mostSpecific($classes)
    {
        $weights = array_map(
            "Denofi\DaemonBundle\System\Daemon\OS::_getAncestorCount",
            $classes
        );
     ...
     * @return string
     */
   static  protected function _getShortHand($class)
    {
     ...
     * @return integer
     */
    static protected function _getAncestorCount($class)
    {
     ...
     * @return array
     */
    static protected function _getAncestors($class)
    {
        $classes = array();
